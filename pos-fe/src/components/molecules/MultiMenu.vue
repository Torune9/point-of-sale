    ...
    <Wrapper>
        <Item v-for="(item, i) in props.menu" :key="i"
            ...
            <RouterLink exact-active-class="bg-accent outline outline-white/50 text-primary/80"  v-if="!item.items" :to="item.path || '#'"
                class="block p-2 hover:bg-accent text-primary/80 hover:text-white transition-colors duration-300 rounded">
            ...
            <div v-if="item.items" class="flex flex-col">
                <div class="rounded flex justify-between items-center p-2 hover:bg-accent/80 text-primary/80 hover:text-white transition-colors duration-300"
                ...
                <transition name="fade">
                    <div v-if="isOpen(i)"
                        ...
                        <RouterLink exact-active-class="bg-accent text-primary/80" v-for="(sub, j) in item.items" :key="j" :to="sub.path || ''"
                            class="p-1.5 hover:bg-accent/50 text-primary/80 hover:text-white transition-colors duration-200 rounded">
                           <span>{{ sub.name }}</span>
                        </RouterLink>
                    </div>
                </transition>
